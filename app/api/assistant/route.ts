...
type ToolName = "search_tickets" | "get_ticket" | "update_ticket_status" | "update_deployment_status" | "update_ticket_states" | "batch_update_ticket_status" | "batch_update_deployment_status" | "batch_update_ticket_states";
...
const tools = [
  ...
  { type:"function", function:{ name:"update_ticket_states", description:"一次同时修改一条工单的处理状态和部署状态。用户在同一句中要求两个状态都改变时必须使用此工具，不能拆成两个单独工具。", parameters:{ type:"object", properties:{ ticket_number:{ type:"string", pattern:"^[0-9]{6}$" }, status:{ type:"string", enum:["pending","processing","completed"] }, deployment_status:{ type:"string", enum:["undeployed","deployed"] } }, required:["ticket_number","status","deployment_status"], additionalProperties:false } } },
  ...
  { type:"function", function:{ name:"batch_update_ticket_states", description:"一次同时修改多条工单的处理状态和部署状态。用户要求多条工单的两个状态都改变时必须使用此工具，整个批次和两个字段只确认一次。", parameters:{ type:"object", properties:{ ticket_numbers:{ type:"array", minItems:1, maxItems:30, uniqueItems:true, items:{ type:"string", pattern:"^[0-9]{6}$" } }, status:{ type:"string", enum:["pending","processing","completed"] }, deployment_status:{ type:"string", enum:["undeployed","deployed"] } }, required:["ticket_numbers","status","deployment_status"], additionalProperties:false } } },
] as const;
...
function cleanConfirmation(value:unknown):Confirmation | null {
  ...
  if (!(tool === "update_ticket_status" || tool === "update_deployment_status" || tool === "update_ticket_states" || tool === "batch_update_ticket_status" || tool === "batch_update_deployment_status" || tool === "batch_update_ticket_states") || !args || typeof args !== "object") return null;
...
async function prepareConfirmation(name:ToolName, args:Record<string, unknown>, user:NonNullable<Awaited<ReturnType<typeof currentUser>>>) {
  if (name === "batch_update_ticket_status" || name === "batch_update_deployment_status" || name === "batch_update_ticket_states") {
    ...
    if (name === "batch_update_ticket_states") {
      const status = cleanString(args.status) as TicketRecord["status"];
      if (!Object.hasOwn(statusLabels, status)) return { error:"处理状态无效。" };
      return { confirmation:{ tool:name, arguments:{ ticket_numbers:numbers, status, deployment_status:deployment } } satisfies Confirmation, message:`确认一次性将以下 ${numbers.length} 条工单同时修改为“${statusLabels[status]}”和“${deploymentLabels[deployment]}”？\n${list}` };
    }
  ...
  if (name === "update_ticket_states") {
    const status = cleanString(args.status) as TicketRecord["status"];
    const deployment = cleanString(args.deployment_status) as TicketRecord["deploymentStatus"];
    if (!Object.hasOwn(statusLabels, status)) return { error:"处理状态无效。" };
    if (!Object.hasOwn(deploymentLabels, deployment)) return { error:"部署状态无效。" };
    return { confirmation:{ tool:name, arguments:{ ticket_number:number, status, deployment_status:deployment } } satisfies Confirmation, message:`确认将工单 #${number} 同时从“${statusLabels[ticket.status]} / ${deploymentLabels[ticket.deployment_status]}”修改为“${statusLabels[status]} / ${deploymentLabels[deployment]}”？` };
  }
...
async function executeConfirmation(confirmation:Confirmation, user:NonNullable<Awaited<ReturnType<typeof currentUser>>>) {
  if (confirmation.tool === "batch_update_ticket_status" || confirmation.tool === "batch_update_deployment_status" || confirmation.tool === "batch_update_ticket_states") {
    ...
    if (!Object.hasOwn(deploymentLabels, deployment)) return { error:"部署状态无效。" };
    if (confirmation.tool === "batch_update_ticket_states") {
      const status = cleanString(confirmation.arguments.status) as TicketRecord["status"];
      if (!Object.hasOwn(statusLabels, status)) return { error:"处理状态无效。" };
      const completed = status === "completed"; const now = Date.now();
      await DB.batch(rows.map((ticket) => DB.prepare("UPDATE tickets SET status = ?, completed = ?, completed_at = ?, deployment_status = ? WHERE id = ?").bind(status, completed ? 1 : 0, completed ? now : null, deployment, ticket.id)));
      for (const ticket of rows) await writeLog(user, "AI助手批量同时更新两个状态", "工单", ticket.id, `编号：${ticket.ticket_number}；处理状态：${status}；部署状态：${deployment}；共${rows.length}条`);
      return { message:`已一次性将 ${rows.length} 条工单同时更新为“${statusLabels[status]}”和“${deploymentLabels[deployment]}”。` };
    }
  ...
  if (!ticket) return { error:"未找到该工单，或当前账号无权修改。" };
  const { DB } = appEnv();
  if (confirmation.tool === "update_ticket_states") {
    const status = cleanString(confirmation.arguments.status) as TicketRecord["status"];
    const deployment = cleanString(confirmation.arguments.deployment_status) as TicketRecord["deploymentStatus"];
    if (!Object.hasOwn(statusLabels, status)) return { error:"处理状态无效。" };
    if (!Object.hasOwn(deploymentLabels, deployment)) return { error:"部署状态无效。" };
    const completed = status === "completed";
    await DB.prepare("UPDATE tickets SET status = ?, completed = ?, completed_at = ?, deployment_status = ? WHERE id = ?").bind(status, completed ? 1 : 0, completed ? Date.now() : null, deployment, ticket.id).run();
    await writeLog(user, "AI助手同时更新两个状态", "工单", ticket.id, `编号：${number}；处理状态：${status}；部署状态：${deployment}`);
    return { message:`工单 #${number} 已同时更新为“${statusLabels[status]}”和“${deploymentLabels[deployment]}”。` };
  }
...
async function prepareRecentBatchFollowUp(input:string, recent:Confirmation|null, user:NonNullable<Awaited<ReturnType<typeof currentUser>>>) {
  ...
  const deployment = targetDeploymentStatus(input);
  const status = targetTicketStatus(input);
  if (deployment && status) return prepareConfirmation(ticketNumbers.length > 1 ? "batch_update_ticket_states" : "update_ticket_states", ticketNumbers.length > 1 ? { ticket_numbers:ticketNumbers, status, deployment_status:deployment } : { ticket_number:ticketNumbers[0], status, deployment_status:deployment }, user);
...
export async function POST(request:Request) {
  ...
  const body = await request.json() as { message?:unknown; history?:unknown; confirmation?:unknown; recentAction?:unknown; resultContext?:unknown };
  ...
  try {
    for (let turn = 0; turn < 5; turn += 1) {
      ...
      const statusMutation = parsedCalls.find(({ call }) => call.function.name === "update_ticket_status" || call.function.name === "batch_update_ticket_status");
      const deploymentMutation = parsedCalls.find(({ call }) => call.function.name === "update_deployment_status" || call.function.name === "batch_update_deployment_status");
      if (statusMutation && deploymentMutation) {
        const statusNumbers = cleanTicketNumbers(statusMutation.args.ticket_numbers); const statusSingle = cleanString(statusMutation.args.ticket_number, 6); if (!statusNumbers.length && /^\d{6}$/.test(statusSingle)) statusNumbers.push(statusSingle);
        const deploymentNumbers = cleanTicketNumbers(deploymentMutation.args.ticket_numbers); const deploymentSingle = cleanString(deploymentMutation.args.ticket_number, 6); if (!deploymentNumbers.length && /^\d{6}$/.test(deploymentSingle)) deploymentNumbers.push(deploymentSingle);
        if (statusNumbers.length && statusNumbers.length === deploymentNumbers.length && statusNumbers.every((number) => deploymentNumbers.includes(number))) {
          const pending = await prepareConfirmation(statusNumbers.length > 1 ? "batch_update_ticket_states" : "update_ticket_states", statusNumbers.length > 1 ? { ticket_numbers:statusNumbers, status:statusMutation.args.status, deployment_status:deploymentMutation.args.deployment_status } : { ticket_number:statusNumbers[0], status:statusMutation.args.status, deployment_status:deploymentMutation.args.deployment_status }, user);
          return Response.json({ ...pending, pageFilter, resultContext }, { status:"error" in pending ? 400 : 200 });
        }
      }
      ...
      for (const call of assistant.tool_calls) {
        ...
        if (call.function.name === "update_ticket_status" || call.function.name === "update_deployment_status" || call.function.name === "update_ticket_states" || call.function.name === "batch_update_ticket_status" || call.function.name === "batch_update_deployment_status" || call.function.name === "batch_update_ticket_states") {
        ...
        if ("error" in result) verifiedReadMessage = result.error;
          ...
          if ("count" in result) { resultContext = { ticketNumbers:result.tickets.map((ticket) => ticket.ticketNumber) }; verifiedReadMessage = result.count ? `已从数据库检索到 ${result.count} 条符合条件的工单，首页列表已同步更新。` : "数据库中没有符合条件的工单，首页列表已同步显示为空。"; }
          else if (result.found) {
            const ticket = result.ticket;
            resultContext = { ticketNumbers:[ticket.ticketNumber] };
