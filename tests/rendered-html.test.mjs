...
test("首页侧边AI助手通过受控工具检索、联动页面并在确认后修改状态", async () => {
  ...
  assert.match(route, /DB\.batch\(rows\.map/);
  ...
  assert.match(route, /published_date/);
  assert.match(route, /verifiedReadMessage/);
  assert.match(route, /已从数据库检索到/);
  assert.match(route, /首页列表已同步更新/);
  assert.match(route, /feedbackDate:dateKeyFormatter\.format\(ticket\.scheduledAt\)/);
  assert.match(route, /publishedAt:publishedTimeFormatter\.format\(ticket\.createdAt\)/);
